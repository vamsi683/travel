<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
    ...
    * @return mixed
    */
    public function add_column($column, $content, $match_replacement = NULL,$callback=array())
    {
      $this->add_columns[$column] = array('content' => $content, 'replacement' => $this->explode(',', $match_replacement));
		//print_r( $this->add_columns);
		$has_callbacks = !empty($callback) ? true : false;
		if( $has_callbacks  )
		{
			$this->callback_column[$column] = $callback;
			//print_r($this->callback_column);
		}
    ...
    protected function produce_output($charset)
    {
      ...
      foreach($rResult->result_array() as $row_key => $row_val)
      {
        ...
        foreach($this->add_columns as $field => $val)
		{
		  ...
		  if( $has_callbacks && isset($this->callback_column[$field]) )
					$aaData[$row_key][$field] = call_user_func($this->callback_column[$field], $this->exec_replace($val, $aaData[$row_key]), $row_val);
		}
